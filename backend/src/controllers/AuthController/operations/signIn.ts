...
import { db } from "../../../db";
import { comparePassword, createToken, hashPassword, userFromRequest } from "../../../global/hashing";
...
export async function signIn(request: Request, response: Response) {
  ...
  const newUser = db.users().create({ username, password: await hashPassword(password) })
